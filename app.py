...
from flask import Flask, render_template, request, redirect, url_for, session, flash, jsonify,send_file,Response,send_from_directory,render_template_string
...
def upload_to_ipfs(file):
    ...
    print("Pinata Response:", response_data)

    if "IpfsHash" not in response_data:
        raise Exception(f"Failed to upload to IPFS: {response_data.get('error', 'Unknown error')}")

    return response_data["IpfsHash"]


def write_to_blockchain(ipfs_hash, max_attempts=3, wait_time=30):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            try:
                ...
                print(f"Transaction not mined within 120 seconds. Attempt {attempt + 1}/{max_attempts}")
                time.sleep(wait_time)
                continue
        
        except Exception as e:
            print(f"Error in attempt {attempt + 1}/{max_attempts}: {str(e)}")
            if attempt < max_attempts - 1:
                time.sleep(wait_time)
            else:
                raise Exception(f"Failed to write to blockchain after {max_attempts} attempts: {str(e)}")

    raise Exception(f"Failed to write to blockchain after {max_attempts} attempts")
    ...
    try:
        ...
        if version is None:
            if not file['versions']:
                return jsonify({"error": "No versions available for this file"}), 404
...
def view_pdf(file_id):
    try:
        ...
        print(f"Unexpected error in view_pdf: {str(e)}")
        return jsonify({"error": f"An unexpected error occurred: {str(e)}"}), 500
